...
@section('content')
                    ...
                    <div class="cheque-page">
                        <form action="{{ route('cheque.store') }}" method="POST" id="cashForm">
                            ...
                            <table class="cheque-table">
                                ...
                                <tbody id="cashEntries"></tbody>
                                <tfoot>
                                    <tr class="summary-row">
                                        <td colspan="5"><strong>Total</strong></td>
                                        <td id="runningBalance">0.00</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
    ...
    {{-- <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const parties =
            @json($parties ?? []); // e.g. {1: {name:'Waqas', type:'supplier'}, 2:{name:'Hamza', type:'dealer'}}
                    ...
                    <td class="position-relative">
                        <div class="party-cell">
                            ...
                            <div class="d-flex align-items-center mt-1 party-search-wrapper">
                                <input type="text" 
                                    class="form-control form-control-sm party-input flex-fill" 
            ...
            function setupPartySearch(row) {
                ...
                // 🔘 Toggle Supplier/Dealer
                sdButtons.forEach(btn => {
                    btn.addEventListener('click', function() {
                        const expBtn = row.querySelector('.exp-btn');
                        if (expBtn && expBtn.classList.contains('btn-warning')) return;

                        sdButtons.forEach(b => b.classList.remove('active', 'btn-primary',
                            'btn-info'));
                        this.classList.add('active', this.dataset.type === 'supplier' ?
                ...
                partyInput.addEventListener('input', function() {
                    ...
                    if (filtered.length === 0) {
                        suggestionList.innerHTML =
                            '<li class="list-group-item text-muted py-1 px-3">No results</li>';
                    ...
                    filtered.forEach(([id, data]) => {
                        ...
                        li.dataset.id = id; // ✅ store id as data attribute

                        li.addEventListener('click', () => {
                            const selectedId = parseInt(li.dataset
                            .id); 
                            partyInput.value = data.name;
                            partyIdInput.value = selectedId; // ✅ correct numeric ID
                            partyTypeInput.value = currentType;
                            suggestionList.innerHTML = '';
                            suggestionList.style.display = 'none';

                            console.log("✅ Selected:", data.name, "| ID:", selectedId,
                                "| Type:", currentType);
                        });
